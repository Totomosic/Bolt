...
namespace Aimbooster
{
	...
	public:
		void Init() override
		{
			...
			Camera* mainCamera = scene->CreateCamera(PrimaryWindow->GetFramebuffer().ViewFrustum(-0.1f, 100), ProjectionType::Orthographic);
			...
			scoreText = mainLayer->UI().AddElement(std::make_unique<Text>("Score: 0", arialFont, Color::White, Transform({ Width() / 2, Height() - 30, -5 }), AlignH::Center));
		...
		void Update() override
		{
			...
			Text* scoretext = mainLayer->UI().GetElementById<Text>(scoreText);
