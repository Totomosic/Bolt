...
namespace DND
{
	...
	void TileMotion::Update()
	{
		if (IsMoving())
		{
			...
			Vector3f targetPosition = GameManager::Get().MapManager().GetMap(gameObject()->Components().GetComponent<TileMapTracker>().MapId()).WorldPositionOfTile(m_TargetTile.x, m_TargetTile.y) + m_Transform->PositionOffset();
	...
	void TileMotion::SetTargetTile(const Tile& targetTile)
	{
		...
		{
			float mapWidth = GameManager::Get().MapManager().GetMap(gameObject()->Components().GetComponent<TileMapTracker>().MapId()).Width();
			float mapHeight = GameManager::Get().MapManager().GetMap(gameObject()->Components().GetComponent<TileMapTracker>().MapId()).Height();
