...
namespace Bolt
{
	...
	public:
		...
		static void Text(const blt::string& text, const Font* font, float x, float y, float z, const Color& color = Color::White, AlignH horizontalAlign = AlignH::Center, AlignV verticalAlign = AlignV::Center, id_t layerId = 0);
		static void Image(float x, float y, float z, float w, float h, const Texture2D* texture, const Quaternion& orientation = Quaternion::Identity(), id_t layerId = 0);
