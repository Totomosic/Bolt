...
namespace Bolt
{
	...
	template<>
	inline void UploadShaderValue(const Shader& shader, int location, const AssetHandle<Texture2D>& value, int index)
	{
		if (value != nullptr)
		{
	...
	template<>
	inline void UploadShaderValue(const Shader& shader, int location, const AssetHandle<Font>& value, int index)
	{
		if (value != nullptr)
		{
			shader.SetUniform(location, index);
