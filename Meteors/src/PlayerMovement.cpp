...
namespace Meteors
{
	...
	void PlayerMovement::Update()
	{
		...
		if (Input::Get().KeyDown(Keycode::A))
		{
			...
			m.Mesh.Materials[0]->GetShader().Link("Texture", ResourceManager::Get().Get<Texture2D>(LeftFacingCharacterTexture));
		}
		else if (Input::Get().KeyDown(Keycode::D))
		{
			m_Velocity.x = m_Speed;
			m.Mesh.Materials[0]->GetShader().Link("Texture", ResourceManager::Get().Get<Texture2D>(RightFacingCharacterTexture));
		}
		else
		{
			m_Velocity.x = 0;
